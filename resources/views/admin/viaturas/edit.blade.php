...
@section('content')
    <div class="box box-widget center-block" style="max-width: 800px;" >
        ...
        <!-- /.box-header -->
        <!-- form start -->
        <form role="form" action="{{route('viaturas.update', $viatura->id)}}" method="POST">            
           {{method_field('PUT')}}
           {{ csrf_field() }}
          <div class="box-body">
            <div class="form-group has-feedback {{ $errors->has('placa') ? 'has-error' : '' }}">
             ...
             <div class="form-group">
                ...
                @foreach ($marcas as $marca)
                    <option value="{{ $marca->id }}" {{ $viatura->modelo->marca->id === $marca->id ? "selected" : "" }}>{{ $marca->nome}}</option>
                @endforeach
                </select>
            </div>
            ...
            <div class="form-group has-feedback {{ $errors->has('id_modelo') ? 'has-error' : '' }}">
                ...
                @foreach ($modelos as $modelo)
                    <option value="{{ $modelo->id }}" {{ $viatura->modelo->id === $modelo->id ? "selected" : "" }}>{{ $modelo->nome}}</option>
                @endforeach
                </select>
            ...
            <div class="form-group has-feedback {{ $errors->has('ano') ? 'has-error' : '' }}">
                ...
                <input type="number" name="ano" id="ano" class="form-control" value="{{ $viatura->ano }}">
